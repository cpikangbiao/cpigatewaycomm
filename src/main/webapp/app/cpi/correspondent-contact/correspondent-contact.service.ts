...
export class CorrespondentContactService {
    ...
    create(correspondentContact: CorrespondentContact): Observable<EntityResponseType> {
        ...
        return this.http.post<CorrespondentContact>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    update(correspondentContact: CorrespondentContact): Observable<EntityResponseType> {
        const copy = this.convert(correspondentContact);
        return this.http.put<CorrespondentContact>(this.resourceUrl, copy, { observe: 'response' })
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    find(id: number): Observable<EntityResponseType> {
        return this.http.get<CorrespondentContact>(`${this.resourceUrl}/${id}`, { observe: 'response'})
            .map((res: EntityResponseType) => this.convertResponse(res));
    }

    query(req?: any): Observable<HttpResponse<CorrespondentContact[]>> {
        const options = createRequestOption(req);
        return this.http.get<CorrespondentContact[]>(this.resourceUrl, { params: options, observe: 'response' })
            .map((res: HttpResponse<CorrespondentContact[]>) => this.convertArrayResponse(res));
    }
